...
| Initial size | Reported by the reproducible notebook when the API request is run. |
| Final analytic size | Reported by the reproducible notebook after analytic filters are applied. |
| Missing/excluded values | The notebook reports missing values by field and the number excluded by the analytic filters. |
...
The notebook creates three weighted-median visualizations. Replace the placeholders below with the real images after running it.
...
Figure 2 shows higher weighted median wage and salary income for men in every education group. The smallest observed difference is among people with high school education or less (about $12,000), while the largest is among people with graduate or professional degrees (about $35,000). The gap does not disappear at higher education levels in this descriptive comparison.
...
Figure 3 shows an observed difference in every age group. The difference is smallest among ages 25-34 (about $8,000) and largest among ages 55-64 (about $18,000). Across the groups shown, the chart suggests that the observed difference becomes larger later in the working-age range.
...
AI was utilized to properly organize and format the information on this page.
